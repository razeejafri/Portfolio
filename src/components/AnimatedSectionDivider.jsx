...
export default function AnimatedSectionDivider({ variant = 'azure-ribbon', className = '' }) {
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* Wave Variant 1: Pearl Liquid Silk Ribbon (Inspired by the white/pearl wave panels in image) */}
      {variant === 'pearl-wave' && (
        <div className="relative w-full">
          {/* Ambient Glow Aura Behind Crest */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-16 bg-cyan-400/10 blur-2xl rounded-full" />
          ...
          >
            <defs>
              {/* Pearl gradient with specular sheen */}
              <linearGradient id={`${uid}-pearlRibbonGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="rgba(255,255,255,0.02)" />
                <stop offset="25%" stopColor="rgba(248,250,252,0.12)" />
                <stop offset="50%" stopColor="rgba(255,255,255,0.22)" />
                <stop offset="75%" stopColor="rgba(224,242,254,0.14)" />
      ...
      {/* Wave Variant 2: Azure Liquid Ribbon (Electric Cyan & Royal Blue in image) */}
      {(variant === 'azure-ribbon' || variant === 'cyan' || variant === 'pearl') && (
        <div className="relative w-full">
          {/* Ambient Blue Back-Glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-20 bg-blue-600/15 blur-3xl rounded-full" />
          ...
          >
            <defs>
              ...
              <linearGradient id={`${uid}-azureRibbonGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="rgba(14,165,233,0.01)" />
                <stop offset="25%" stopColor="rgba(37,99,235,0.18)" />
                <stop offset="50%" stopColor="rgba(56,189,248,0.28)" />
                <stop offset="75%" stopColor="rgba(29,78,216,0.18)" />
                <stop offset="100%" stopColor="rgba(14,165,233,0.01)" />
              </linearGradient>

              {/* Neon Cyan Specular Crest */}
              <linearGradient id={`${uid}-azureCrestGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="rgba(6,182,212,0)" />
                <stop offset="25%" stopColor="rgba(56,189,248,0.6)" />
                <stop offset="50%" stopColor="#38BDF8" />
                <stop offset="75%" stopColor="rgba(96,165,250,0.6)" />
                <stop offset="100%" stopColor="rgba(6,182,212,0)" />
              </linearGradient>

              {/* Deep Cobalt Under-Wave */}
              <linearGradient id={`${uid}-azureUnderGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="transparent" />
                <stop offset="50%" stopColor="rgba(30,58,138,0.25)" />
                <stop offset="100%" stopColor="transparent" />
              </linearGradient>
            </defs>

            {/* Back Deep Cobalt Wave */}
            <path
              d="M0,60 C380,10 700,105 1060,35 C1240,0 1380,70 1440,55 L1440,120 L0,120 Z"
              fill={`url(#${uid}-azureUnderGrad)`}
            />

            {/* Main Azure Liquid Ribbon */}
            <path
              d="M0,70 C360,15 680,110 1020,40 C1220,-5 1360,75 1440,65 L1440,120 L0,120 Z"
              fill={`url(#${uid}-azureRibbonGrad)`}
            />

            {/* Neon Cyan Beveled Crest Edge */}
            <path
              d="M0,70 C360,15 680,110 1020,40 C1220,-5 1360,75 1440,65"
              stroke={`url(#${uid}-azureCrestGrad)`}
              strokeWidth="2"
              strokeLinecap="round"
              className="drop-shadow-[0_0_14px_rgba(56,189,248,0.85)]"
            />
          </svg>
        </div>
      )}

      {/* Wave Variant 3: Aurora Wave (Cyan + Indigo + Violet Iridescent) */}
      {(variant === 'aurora-wave' || variant === 'aurora' || variant === 'rose') && (
        <div className="relative w-full">
          {/* Ambient Multi-Chromatic Glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-20 bg-gradient-to-r from-cyan-500/10 via-violet-600/15 to-blue-500/10 blur-3xl rounded-full" />
          ...
          >
            <defs>
              ...
              <linearGradient id={`${uid}-auroraCrestGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                ...
                <stop offset="75%" stopColor="rgba(56,189,248,0.8)" />
                <stop offset="100%" stopColor="rgba(56,189,248,0)" />
              </linearGradient>
            </defs>
      ...
      {/* Wave Variant 4: Emerald / Cyan Wave */}
      {(variant === 'emerald' || variant === 'amber') && (
        <div className="relative w-full">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4/5 h-20 bg-emerald-500/10 blur-3xl rounded-full" />
          ...
          >
            <defs>
              <linearGradient id={`${uid}-emeraldRibbonGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                ...
                <stop offset="40%" stopColor="rgba(16,185,129,0.16)" />
                <stop offset="60%" stopColor="rgba(6,182,212,0.22)" />
            ...
            <path
              ...
              strokeWidth="2"
              strokeLinecap="round"
              className="drop-shadow-[0_0_12px_rgba(52,211,153,0.7)]"
            />
          </svg>
        </div>
      )}
          ...
          >
            <defs>
              <linearGradient id={`${uid}-pearlRevGrad`} x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="rgba(255,255,255,0.02)" />
                <stop offset="30%" stopColor="rgba(248,250,252,0.14)" />
                <stop offset="50%" stopColor="rgba(255,255,255,0.24)" />
                <stop offset="70%" stopColor="rgba(224,242,254,0.14)" />
                <stop offset="100%" stopColor="rgba(255,255,255,0.02)" />
              </linearGradient>

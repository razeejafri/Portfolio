...
export default function ScrollProgressBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[100] h-[48px] pointer-events-none select-none">
      ...
      <motion.div
        className="absolute bottom-0 left-0 h-[2.5px] bg-gradient-to-r from-[#1877F2] via-[#38BDF8] to-[#818CF8] shadow-[0_0_14px_rgba(56,189,248,0.95)] rounded-r-full"
        style={{ width: barWidth }}
      >
        {/* Trailing soft cyan ambient glow */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#38BDF8]/30 to-[#38BDF8]/60 blur-[3px]" />
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 flex items-center justify-center pointer-events-none">
          <div className="w-2.5 h-2.5 rounded-full bg-cyan-200 shadow-[0_0_12px_#38bdf8] animate-pulse" />
          <div className="absolute w-6 h-6 rounded-full bg-cyan-400/40 blur-[3px]" />
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="absolute -bottom-[2px] left-1/2 -translate-x-1/2 w-5 h-1 bg-cyan-300 rounded-full blur-[1px] opacity-85 pointer-events-none" />
        ...
        <AnimatePresence>
          {(isHovered || isRunning || meowText || isAtEnd) && (
            <motion.div
              ...
              className="absolute top-[44px] whitespace-nowrap px-2.5 py-0.5 rounded-full text-[10px] font-mono font-semibold tracking-wider bg-[#060B18]/95 text-[#38BDF8] border border-cyan-400/40 shadow-[0_4px_16px_rgba(56,189,248,0.35)] backdrop-blur-md pointer-events-none flex items-center gap-1.5 z-30"
            >
              {/* Pointing caret */}
              <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 rotate-45 bg-[#060B18] border-l border-t border-cyan-400/40" />

              {meowText ? (
                <span className="relative z-10 text-cyan-200 font-sans">{meowText}</span>
              ) : isAtEnd ? (
                <div className="relative z-10 flex items-center gap-1.5 text-amber-300">
                  <span>🏆</span>
                  <span>100% Completed!</span>
                  <span className="text-[9px] text-cyan-300">Purrr~</span>
                </div>
              ) : isAtStart ? (
                <div className="relative z-10 flex items-center gap-1.5 text-slate-300">
